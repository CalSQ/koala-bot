    ...
    switch (subCommand) {
        case ("database"): {
            ...
            if (user) {
                ...
                let memberData = await Member.getByIds(interaction.guild.id, user.id)
                ...
                let guildData = await Guild.getById(interaction.guild.id)
        ...
        case ("log"): {
            ...
            console.log(type)
            const guildData = await Guild.getById(interaction.guild.id)
            guildData.values.options[type] = channel ? channel.id : undefined

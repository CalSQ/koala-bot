...
export default class ApplicationClient extends Client {
    ...
    readonly config: Config = config as Config;
    ...
    public async registerEvents() {
        ...
        import("../events").then((eventsModule) => {
            ...
            for (const event of events) {
                ...
                if (event.once) {
                    this.once(event.key, (...args) => {
                        ...
                        try { event.callback(((client) => { return { client, log } })(this), ...args) } 
                        catch (err) { log("[ ⚠ Uncaught Error ]", err) };
                    })
                } else {
    ...
    public async registerCommands() {
        ...
        import("../commands").then(({ slashCategory, contextCategory }) => {
            const slashCommands = slashCategory.map(category => category.commands).flat()
            for (const command of slashCommands) {
                this.commands.set(command.build.name, command)
            }
            const contextCommands = contextCategory.map(category => category.commands).flat()
    ...
    public async registerInteractions() {
        ...
        import("../interactions").then(({ buttonCategory, menuCategory, modalCategory }) => {
            const buttons = buttonCategory.map(category => category.interactions).flat()
            for (const button of buttons) {
                this.buttons.set(button.name, button)
            }
            const menus = menuCategory.map(category => category.interactions).flat()
            for (const menu of menus) {
                this.selectMenus.set(menu.name, menu)
            }
            const modals = modalCategory.map(category => category.interactions).flat()
    ...
    public async deploy(refresh: boolean = false) {
        ...
        const applicationCommandEndpoint = Routes.applicationCommands(this.user.id);
        const applicationCommands = await rest.put(applicationCommandEndpoint, { body: globalCommands }).catch(err => {
            console.error("[ DEPLOY ERROR ]:",err);
        }) as ApplicationCommand[];
        console.log(`[ DEPLOY ]: Deployed ${applicationCommands.length} / ${globalCommands.length} global command(s)`);

        // Guild (Developer) Commands
        if (!this.config.info.developerGuild) return console.warn("[ DEPLOY ]: Guild ID missing (Developer Commands)");
        const applicationGuildCommandEndpoint = Routes.applicationGuildCommands(this.user.id, this.config.info.developerGuild);
        const applicationGuildCommands = await rest.put(applicationGuildCommandEndpoint, { body: devCommands }).catch(err => {
            console.error("[ DEPLOY ERROR ]:",err);
        }) as ApplicationCommand[];
        console.log(`[ DEPLOY ]: Deployed ${applicationGuildCommands.length} / ${devCommands.length} developer command(s)`);
    }
